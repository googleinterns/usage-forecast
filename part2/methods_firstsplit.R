...
data_impute_prep_mem <- data_agg_sum[data_agg_sum$date < as.Date("2018-08-01"),]
error_date <- data_impute_prep_mem[data_impute_prep_mem$memory_gib_seconds > 3.5e4, "date"] # 2018-05-01 - 05-03

for (i in 1:length(error_date)) {
  data_agg_sum[data_agg_sum$date == error_date[i], "memory_gib_seconds"] <- 
    mean(data_agg_sum[data_agg_sum$date == error_date[1]-i, "memory_gib_seconds"], 
         data_agg_sum[data_agg_sum$date == error_date[length(error_date)]+i, "memory_gib_seconds"])
  
}

data_check <- data_agg_sum[data_agg_sum$date < as.Date("2019-02-01"),]
error_date <- data_check[data_check$memory_gib_seconds > 5e4, "date"] # 2018-12-28
...
GetLineplot(dataset = data_agg_sum[data_agg_sum$date < change_date,], "cell", filepath = "plots/transform/impute_org_") 
GetLineplot(dataset = data_agg_sum, "cell", filepath = "plots/transform/impute_all_org_") 
...
saveRDS(data_agg_sum_trans_log, "data_agg_sum_trans_log.rds")
# plots of the data after transformation.
...
# Tried stl(), decompose() deseasoning method, but none of them detect seasonality.
...
data_agg_sum_trans_log_deseason <- de_seasonal(data_agg_sum_trans_log, 
...
# data_agg_sum_trans_log_deseason_origin <- Backtransform(data_agg_sum_trans_log_deseason,
...
# 
# 
# plot(data_agg_sum_trans_log_deseason_origin$gcu_seconds, type = "l")
# lines(data_agg_sum$gcu_seconds[2:(nrow(data_agg_sum_trans_log_deseason_origin)+1)], col=2)


# Check box plot after de-seasonality.
# Transformed data.
data_agg_trans_sum_plt_desea <- GetTimeInfo(data_agg_sum_trans_log_deseason)
data_agg_trans_sum_plt_desea <- data_agg_trans_sum_plt_desea[data_agg_trans_sum_plt_desea$date <
                                                               change_date,]
# Plot transformed time series after deseasoning without first week.
...
lines(data_agg_sum_trans_log_deseason$date, data_agg_sum_trans_log_deseason$gcu_seconds, col = 2)

# Check Stationarity after de-seasoning.
features_name <- c("gcu_seconds", "memory_gib_seconds")
pval <- c()
for (i in features_name) {
  data_before_ind <- is.na(data_agg_sum_trans_log_deseason[(season+1):nrow(data_agg_sum_trans_log_deseason),i])
  print(i)
  pval[i] <- tseries::adf.test(data_agg_sum_trans_log_deseason[(season+1):nrow(data_agg_sum_trans_log_deseason),][!data_before_ind,i], alternative = "stationary")$p.value
  acf(data_agg_sum_trans_log_deseason[(season+1):nrow(data_agg_sum_trans_log_deseason),][!data_before_ind,i])
  pacf(data_agg_sum_trans_log_deseason[(season+1):nrow(data_agg_sum_trans_log_deseason),][!data_before_ind,i])
}

### Split data ###
# Get one week time after March 28, 2018 as the comparison set.
data_log_impute <- data_agg_sum_trans_log_deseason[, c("date", features_name)]
hold_date <- 7
...
tuning_set <- train_set_all[train_set_all$date < end_date,]
validation_set <- train_set_all[train_set_all$date >= end_date,]
...
### Check ValidateModel ### 
# initial_data <- data_agg_sum[data_agg_sum$date == (tuning_set[1,]$date-1), ]
# initial_data_16rm <- data_agg_sum[data_agg_sum$date == (tuning_set[365,]$date-1), ]
...
season = 7
cutoff = 365
selected_features <- c("date", features_name)
initial_data <- data_agg_sum[data_agg_sum$date == (tuning_set[1,]$date-1), ]
...
var_pred <- ValidateModel(validate_type = "moving",
                          ...
                          validation_set = holddout_set[holddout_set$date <= change_date,], 
                          validation_interval = 7,
                          metric = "MSE", 
                          is.plot = TRUE, 
                          is.origin = T, 
...
############################################################################
##### Section 4: Results for turning on ThinLTO by default for AutoFDO #####
############################################################################
### Prediction the contextual GCU and Memory for turning on ThinLTO by default for AutoFDO ### 
# Get prediction.
...
actual_data <- data_agg_sum[change_date < data_agg_sum$date &
                              data_agg_sum$date<= change_date + 7, c("gcu_seconds", "memory_gib_seconds", "date")] 

pred_data <- var_pred_est$prediction_org
sum(pred_data$gcu_seconds - actual_data$gcu_seconds) / sum(pred_data$gcu_seconds) # 0.02478519
sum(pred_data$memory_gib_seconds - actual_data$memory_gib_seconds) / sum(pred_data$memory_gib_seconds) #-0.0005277088

# Zoom in plots.
features <- c("gcu_seconds", "memory_gib_seconds")
actual_set <- data_agg_sum[2:nrow(data_agg_sum), ]
var_validate_res <- var_validate$prediction_org
var_validate_ind <- actual_set$date < var_validate_res$date[1]
...
var_pred_res <- rbind(actual_set[var_pred_ind, c(features, "date")], 
                      var_pred_res)
var_pred_res <- var_pred_res[var_pred_res$date != as.Date("2018-03-11"), ]
actual_data_rmoutlier <- actual_set[actual_set$date != as.Date("2018-03-11"), ]
...
GetZoomIn(dataset = var_validate_res, 
          ...
          breakline = as.Date("2018-01-14"),
          features_name = features, features_lab = c("GCU-Seconds", "RAM (GiB) - Seconds")) 

GetZoomIn(dataset = var_pred_res, 
          actual_set = actual_data_rmoutlier, 
          interval = c((nrow(var_pred_res) - 30 - 29): nrow(var_pred_res)),
          breakline = as.Date("2018-02-25"),
          features_name = features, features_lab = c("GCU-Seconds", "RAM (GiB) - Seconds")) 

GetZoomIn(dataset = var_pred_est_res, 
          actual_set = actual_set, 
          interval = c((nrow(var_pred_est_res) - 7 - 29): nrow(var_pred_est_res)),
          breakline = as.Date("2018-03-28"),
          features_name = features, features_lab = c("GCU-Seconds", "RAM (GiB) - Seconds")) 
