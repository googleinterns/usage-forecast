...
GetZoomIn <- function(dataset, actual_set, interval, breakline, features_name, features_lab) {
  par(mfrow=c(2,1))
  for (i in 1:length(features_name)) {
    plot(dataset$date[interval], dataset[interval, features_name[i]], type = "l", 
         col = 2, ylab = features_lab[i], xlab = "Time", main = "Original")
    lines(actual_set$date[interval], 
          actual_set[interval, features_name[i]], col = 1)
    abline(v = breakline, col = 3, lty = 2)
  }
  legend("bottomright", 
         legend = c("Actual Values", "Predicted Values", "Prediction Start Date"), 
         col = 1:3, lty = c(1, 1, 2), cex = 0.8)
}
