...
GetSummary <- function(info_table = info_table_feature3_second){
...
# Return: Saved line graph for feature3, feature1, instance, and feature4
GetLineplot = function(dataset, filepath) {
  gg_feature3 = ggplot(dataset, 
                  aes(x = as.Date(date), y = feature3_seconds)) + 
    geom_line() + 
    xlab("")
  
  
  gg_feature1 = ggplot(dataset, 
                       aes(x = as.Date(date), y = feature1)) + 
    geom_line() + 
    xlab("")
  
  gg_feature2 = ggplot(dataset, 
                        aes(x = as.Date(date), y = feature2)) + 
    geom_line() + 
    xlab("")
  
  gg_feature4 = ggplot(dataset, 
                     aes(x = as.Date(date), y = feature4_gib_seconds)) + 
    geom_line() + 
    xlab("")
  
  ggsave(filename = paste(filepath,"feature3_feature1_linegraph.png", sep = ""), 
         plot = grid.arrange(gg_feature3, gg_feature1),
         width = 10, height = 8, dpi = 100)
  ggsave(filename = paste(filepath,"feature2_feature4_linegraph.png", sep = ""),
         plot = grid.arrange(gg_feature2, gg_feature4),
         width = 10, height = 8, dpi = 100)
}
...
# @tuning_set: dataframe of time series, must include the feature feature3_seconds and feature4_gib_seconds
...
ArimaModel <- function(tuning_set, p = 11, q = 0, 
                       ...
                       y_feature = NULL){
  fit_feature3 <- arima(tuning_set[,"feature3_seconds"], order=c(p, 0, q))
  fit_mem <- arima(tuning_set[,"feature4_gib_seconds"], order=c(p, 0, q))
  pred_feature3 <- predict(fit_feature3, n.ahead = forcast_time)$pred
  pred_mem <- predict(fit_feature3, n.ahead = forcast_time)$pred
  return(list("pred_feature3" = pred_feature3, "pred_mem" = pred_mem))
}
...
BaysianModel <- function(tuning_set,  
  ...
  return(list("pred_feature3" = pred_ls[[1]], "pred_mem" = pred_ls[[2]]))
}
...
#2. predict only feature3 and memeory, and use the previous know other features to get next interval time
...
VarModel <- function(dataset, tuning_interval = 10, lambda = seq(0.0001,0.005, length.out = 20), 
  ...
  return(list("pred_feature3" = pred_normback[,"feature3_seconds" ], "pred_mem" = pred_normback[,"feature4_gib_seconds"], "tuning_parameter" = lambda_min))
}
...
Backtransform <- function(dataset, 
                          ...
                          features = c("feature3_seconds", "feature4_gib_seconds"),
...
# Return the results for feature3 and memeory
ValidateModel <- function(validate_type = "expand",
  ...
  res <- list()
  error <- list("feature3_err" = NULL, "mem_err" = NULL)
  features <- c("feature3_seconds", "feature4_gib_seconds")
  ...
  for (i in 0:validation_times) {
    ...
    if (is.origin) {
      validation_set_pred <- as.data.frame(rbind(as.matrix(tune_set_new[,features]), cbind(res[[i+1]]$pred_feature3, res[[i+1]]$pred_mem)))
      ...
      error$feature3_err[[i+1]] <- data_origin_pred$feature3_seconds - 
        validation_set_new_org$feature3_seconds
      error$mem_err[[i+1]] <- data_origin_pred$feature4_gib_seconds - 
        validation_set_new_org$feature4_gib_seconds
      data_origin_pred_ls[[i+1]] <- data_origin_pred
      
    } else {
      error$feature3_err[[i+1]] <- res[[i+1]]$pred_feature3 - validation_set_new$feature3_seconds
      error$mem_err[[i+1]] <- res[[i+1]]$pred_mem - validation_set_new$feature4_gib_seconds
    }
  }
  
  
  eval_res <- list("feature3" = NULL, "mem" = NULL)
  
  if (metric == "MSE") {
    eval_res$feature3 <- mean(unlist(error$feature3_err)^2)
  ...
  if (is.plot == TRUE) {
    pred_transform <- lapply(res, function(x) cbind(x$pred_feature3, x$pred_mem))
...
GetTunValTable <- function(tuning_set, is.together, lag_num = 18, weight_type = "equal") {
  ...
  x_feature <- c("feature1", "feature2", "feature3_seconds", "feature4_gib_seconds")
  for (i in 1:length(validation_interval)) {
    for (j in 1:length(hold_day)) {
      ...
      mse_var[2*j-1,i] <- var_res_together$MSE$feature3
...
GetModelValTable <- function(model, is.tuning, ...) {
  ...
  for (i in 1:length(validation_interval)) {
    ...
    mse_var[1,i] <- var_res_together$MSE$feature3
