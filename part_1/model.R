...
features_select <- c("feature1", "feature2", "feature3_seconds", 
                     "feature4_gib_seconds", "feature5_gib_seconds")
...
plot(data_org_impute$date, data_org_impute$feature3_seconds, type = "l", col = 2)
tmp2 <- info_table_1day_clean_agg[info_table_1day_clean_agg$date <= end_date,]
lines(tmp2$date, tmp2[, "feature3_seconds"])
...
features_select_feature5_rm <- features_select[1:4]
for (i in features_select_feature5_rm) {
...
# # Check VarModel
# work
y_feature = c("feature3_seconds", "feature4_gib_seconds")
...
res_notogether <- GetTunValTable(tuning_set, is.together = F, lag_num = 15) 
res_notogether_16rm <- GetTunValTable(tuning_set[366:nrow(tuning_set),], is.together = F, lag_num = 18) 
res_together <- GetTunValTable(tuning_set, is.together = T, lag_num = 15) 
res_together_16rm <- GetTunValTable(tuning_set[366:nrow(tuning_set),], is.together = T, lag_num = 18) 
...
rownames(res_tunval_table) <- rep(c("MSE_feature3", "MSE_feature4"),8)
...
res_together_16rm_weighted <- GetTunValTable(tuning_set[366:nrow(tuning_set),], 
                                             is.together = T, lag_num = 25, weight_type = "else") 
...
rownames(model_com_res) <- rep(c("MSE_feature3", "MSE_feature4"),3)
