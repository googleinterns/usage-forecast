rm(list = ls())
setwd("/Volumes/GoogleDrive/My Drive/alsta_analysis_LA/second_split")
...
### Impute data ###
# Remove future data.
end_date <- as.POSIXlt("2018-06-21", tz = "America/Los_Angeles")
...
data_impute_prep_mem[data_impute_prep_mem$memory_gib_seconds > 5e4,] # 2018-05-01
data_agg_sum[as.character(data_agg_sum$date) == "2018-05-01", "memory_gib_seconds"] <- mean(data_agg_sum[as.character(data_agg_sum$date) == "2018-04-30", "memory_gib_seconds"],
                                                                                            data_agg_sum[as.character(data_agg_sum$date) == "2018-05-02", "memory_gib_seconds"])
...
breaktime <- as.POSIXct("2018-06-21", tz = "America/Los_Angeles")
...
data_agg_org_sum_plt <- data_agg_org_sum_plt[data_agg_sum$date <
                                               as.POSIXlt("2018-06-20", tz = "America/Los_Angeles"),]
...
data_agg_sum_trans_log <- data_agg_sum_trans_log[data_agg_sum_trans_log$date <
                                                   as.POSIXlt("2018-07-21", tz = "America/Los_Angeles"),]
...
data_agg_trans_sum_plt_desea <- data_agg_trans_sum_plt[data_agg_trans_sum_plt_desea$date <
                                                         as.POSIXlt("2018-06-20", tz = "America/Los_Angeles"),]
...
hold_date <- 14
end_date <- as.Date(as.POSIXlt("2018-06-21", tz = "America/Los_Angeles")) + hold_date
comparison_set <- data_log_impute[as.Date(data_log_impute$date) < end_date & 
                                    as.Date(data_log_impute$date) >= as.Date(as.POSIXlt("2018-06-21", tz = "America/Los_Angeles")),]
...
train_set_all$date <- as.Date(train_set_all$date)
validation_set_all <- holddout_set[holddout_set$date < as.POSIXct("2018-06-21", tz = "America/Los_Angeles"),]
validation_set_all$date <- as.Date(validation_set_all$date)
...
var_pred <- ValidateModel(validate_type = "moving",
                          ...
                          initial_data = initial_data,
                          tuning_interval = 7, 
                          lambda = seq(0.000025,0.01,0.000025), 
                          hold_day = 14, 
                          lag_num = lag_num, 
                          x_feature = x_feature,
                          is.together = F, 
...
true_trainset_all <- rbind(train_set_all, 
                           holddout_set[holddout_set$date < as.POSIXct("2018-06-21", tz = "America/Los_Angeles"),])
comparison_set$date <- as.Date(comparison_set$date)
...
actual_data <- data_agg_sum[as.POSIXct("2018-06-21")<=data_agg_sum$date &
                              data_agg_sum$date < as.POSIXct("2018-07-05"), c("gcu_seconds", "memory_gib_seconds", "date")] 
...
data_org$date <- as.Date(data_org$date)
...
lines(as.Date(actual_data$date), actual_data$gcu_seconds, col = 2)
...
lines(as.Date(actual_data$date), actual_data$memory_gib_seconds, col = 2)
